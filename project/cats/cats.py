...
def about(subject):
    ...
    def select(text):
        words = split(remove_punctuation(lower(text)))
         
        return any(w in set_subject for w in words)
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    match_words = sum(x == y for x,y in zip(typed_words,source_words))
    accur = match_words / len(typed_words) * 100
    return accur
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        numbers += [(diff,index,word)]
    lowest_diff, _ , lowest_word =  min(numbers,key= lambda x:(x[0],x[1]))
...
def minimum_mewtations(typed, source, limit):
    ...
    if len(typed) == 0:
        return len(source)        
    if len(source) == 0:
        return len(typed) # Base cases should go here, you may add more base cases as needed.
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in timestamps_per_player:
        ...
        for i in range(1, len(player)):
            time_per_player.append(player[i]-player[i-1])
        time.append(time_per_player)
...
def fastest_words(match):
    ...
    amount_player = len(times)
    fastest = [[] for _ in range(amount_player)]
    for i,word in enumerate(words):
    #look for each word,who is the fastest
        word_time =  [times[player][i] for player in range(amount_player)]
        idx = min(range(amount_player),key=lambda x:word_time[x])
