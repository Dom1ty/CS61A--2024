...
class Ant(Insect):
    ...
    def remove_from(self, place):
        if place.ant is self:
            ...
            assert place.ant is None, '{0} is not in {1}'.format(self, place)
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place is not None:
            if place.bees and not place.is_hive and self.lower_bound <= distance <= self.upper_bound:
...
class ContainerAnt(Ant):
    ...
    """
    is_container = True
    #instance varbale,,, ant_contained
    #syntax a = ContainerAnt
    #method container.can_contain 
    #.Store_ant(thing)
     #place.add_insect       
