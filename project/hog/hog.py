...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    mostright = player_score % 10
    second_digit = (opponent_score // 10 ) % 10
    if (opponent_score // 10) <= 0:
        second_digit = 0 
    points = 3*abs(mostright - second_digit)
    return max(points,1)
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    best_score = 0
    for i in range(1,11):
        score = averaged_roll(i,dice)
        if score > best_score or score == (best_score and i < best_num):
            best_num,best_score = i,score
    return best_num
    # END PROBLEM 9
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    potential_score = boar_brawl(score,opponent_score)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    # 如果roll 0可以直接赢，直接roll 0
    if sus_update(0, score, opponent_score) >= 100:
        return 0
    # 如果roll 1可以赢
    if sus_update(1, score, opponent_score) >= 100:
        return 1
    # 如果roll 2可以赢
    if sus_update(2, score, opponent_score) >= 100:
        return 2

    # 如果Boar Brawl可以拿很多分，直接roll 0
    gain = boar_brawl(score, opponent_score)
    if gain >= 6:
        return 0
